<x-layouts::auth :title="__('Register')">
    <div class="flex flex-col gap-6">
        ...
        <form method="POST" action="{{ route('register.store') }}" class="flex flex-col gap-6">
            ...
            <flux:input name="tenant" :label="__('Team')" :value="old('tenant')" type="text" required
                autocomplete="text" placeholder="backend, frontend, etc" />
            <flux:input name="domain" :label="__('Domain')" :value="old('domain')" type="text" required
                autocomplete="text" placeholder="backend, frontend, etc" />
